...
def evaluate_hand(hole_cards, board):
    ...
    all_cards = [card_defs[card] for card in hole_cards + board]
    ...
    # 8: Four of a Kind
    if quads:
        kickers = [v for v in values if v != quads[0]]
        return (8, [quads[0], kickers[0]])
    ...
    # 6: Flush
    if flush_cards:
        return (6, [card["value"] for card in flush_cards[:5]])
    ...
    # 3: Two Pair
    if len(pairs) >= 2:
        kickers = [v for v in values if v != pairs[0] and v != pairs[1]][:1]
...
for seat in active_players:
    ...
    hand_type, values = evaluate_hand(hole_cards, board)
    hand_rankings.append((player_id, seat, hand_type, values))

# Sort by hand strength (hand type, then values)
def compare_hands(hand1, hand2):
    _, _, type1, values1 = hand1
    _, _, type2, values2 = hand2
...
for i, (player_id, _, hand_type, _) in enumerate(winners):
    ...
    print(f"{player_id} wins {award} chips with {hand_name}", file=sys.stderr)
