...
# Check if all-in is possible
if world["game_state"]["phase"] in ["waiting", "ready_to_start", "showdown"]:
    print(f"Cannot go all-in during {world['game_state']['phase']} phase")
    sys.exit(1)
...
# Calculate how much more they're adding
player_current_bet = player_seat["current_bet"]
additional_amount = player_chips

# Update player's state
world["players"][player]["chips"] = 0
player_seat["current_bet"] += additional_amount
player_seat["total_bet_this_round"] += additional_amount
player_seat["has_acted"] = True
world["game_state"]["pot"] += additional_amount

# If this is larger than the current bet, update current bet
if player_seat["current_bet"] > world["game_state"]["current_bet"]:
    world["game_state"]["current_bet"] = player_seat["current_bet"]
...
print(f"{player} goes all-in with {additional_amount} chips!")
