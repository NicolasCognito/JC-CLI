...
# Check if folding is even possible
if world["game_state"]["phase"] in ["waiting", "ready_to_start", "showdown"]:
    print(f"Cannot fold during {world['game_state']['phase']} phase")
    sys.exit(1)
...
if active_positions:
    ...
    current_pos_index = active_positions.index(player_seat["position"]) if player_seat["position"] in active_positions else -1
    
    if current_pos_index != -1 and len(active_positions) > 1:
        next_pos_index = (current_pos_index + 1) % len(active_positions)
        world["game_state"]["current_turn"] = active_positions[next_pos_index]
    else:
        # Only one player left or other error, let rules handle it
        pass
